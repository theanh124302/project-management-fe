...
const TaskDetail = () => {
  ...
  useEffect(() => {
    const fetchTaskDetail = async () => {
      try {
        const response = await axiosInstance.get(`/api/v1/task/findById?id=${taskId}`);
        ...
        // Fetch API details
        if (taskData.apiId) {
          const apiResponse = await axiosInstance.get(`/api/v1/api/findById?id=${taskData.apiId}`);
    ...
    const fetchAssignedUsers = async () => {
      try {
        const response = await axiosInstance.get(`/api/v1/user/findByTaskId/${taskId}`);
  ...
  const handleAssign = async () => {
    try {
      await axiosInstance.post(`/api/v1/task/assignByUsername`, null, {
        params: { taskId, username, assignerId: userId },
      });
      const response = await axiosInstance.get(`/api/v1/task/findById?id=${taskId}`);
      setTask(response.data.data);
      const usersResponse = await axiosInstance.get(`/api/v1/user/findByTaskId/${taskId}`);
  ...
  const handleUnassign = async (username) => {
    try {
      await axiosInstance.post(`/api/v1/task/unassignByUsername`, null, {
  ...
  const handleUpdateStatus = async () => {
    try {
      ...
      const response = await axiosInstance.post(`/api/v1/task/update`, updatedTask);
      setTask(response.data.data);
    } catch (error) {
  ...
  const handleUpdateTask = async () => {
    try {
      await axiosInstance.post(`/api/v1/task/update`, {
        ...task,
        ...newTask,
      });
      setShowForm(false);
  ...
  const handleDeleteTask = async () => {
    try {
      await axiosInstance.delete(`/api/v1/task/delete`, { data: { id: taskId } });
