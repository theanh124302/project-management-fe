...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      try {
        const projectResponse = await axiosInstance.get(`/api/v1/project/findById?id=${projectId}`);
        ...
        const leaderResponse = await axiosInstance.get(`/api/v1/user/findById/${projectResponse.data.data.leaderId}`);
        setLeaderName(leaderResponse.data.data.name);
        
        const membersResponse = await axiosInstance.get(`/api/v1/user/findByProjectId/${projectId}`);
  ...
  const handleAddOrEditMember = async () => {
    try {
      await axiosInstance.post(`/api/v1/project/assignUserByUsername`, null, {
        params: {
          projectId,
  ...
  const handleDeleteMember = async (username) => {
    try {
      await axiosInstance.post(`/api/v1/project/removeUserByUsername`, null, {
        params: {
          projectId,
          username,
          deleterId: userId,
  ...
  const handleUpdateProjectDates = async () => {
    try {
      ...
      const response = await axiosInstance.post(`/api/v1/project/update`, updatedProject, {
        params: { userId }
      });
  ...
  const handleLeaveProject = async () => {
    try {
      await axiosInstance.post(`/api/v1/project/leaveProject`, null, {
        params: {
          projectId,
          userId,
        },
  ...
  const handleDeleteProject = async () => {
    try {
      await axiosInstance.delete(`/api/v1/project/delete`, { params: { id: project.id, userId } });
      navigate('/projectList');
    } catch (error) {
