...
const DatabaseServerList = () => {
  ...
  const fetchServers = async () => {
    try {
      const response = await axiosInstance.get(`/api/v1/database-server/findAllByProjectId?projectId=${projectId}&page=0&size=100`);
  ...
  const handleAddServer = async () => {
    try {
      await axiosInstance.post(`/api/v1/database-server/create`, {
        ...newServer,
        projectId: projectId,
      });
      setShowForm(false);
